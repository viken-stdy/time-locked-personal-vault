...
contract TimeLockedVault {
    ...
    uint256 public immutable lockDuration;    // Lama kunci dalam satuan detik
    uint256 public unlockTime;                // Waktu tepat dana bisa ditarik
    bool public fundsDeposited;               // Status: ada dana atau tidak
    ...
    modifier isUnlocked() {
        require(block.timestamp >= unlockTime, "Error: Dana masih dalam masa kunci");
        _;
    }

    modifier noActiveDeposit() {
        require(!fundsDeposited, "Error: Sudah ada dana, tarik dulu sebelum setor lagi");
        _;
    }

    // --- Konstruktor Kontrak ---
    constructor(uint256 _lockDuration) {
        require(_lockDuration > 0, "Error: Lama waktu kunci harus lebih dari 0");
        owner = msg.sender;
    ...
    function deposit() external payable onlyOwner noActiveDeposit {
        require(msg.value > 0, "Error: Jumlah setoran tidak boleh kosong");
    ...
    function withdraw() external onlyOwner isUnlocked {
        ...
        // Reset status setelah penarikan
